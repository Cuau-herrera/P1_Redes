...
/* Functions added for the node registry */
static int8_t Coordinator_FindByExtended(uint64_t extendedAddress);
static uint8_t Coordinator_RegisterNode(uint64_t extendedAddress, uint8_t capabilityInfo, uint16_t *assignedShortAddress);
static void Coordinator_PrintNode(uint8_t nodeIndex);
...
*************************************************************************************
************************************************************************************/
/* Maximum number of devices that can be registered by the Coordinator. */
#define mCoordinatorMaxNodes_c 5
typedef struct
{
...
static const uint16_t mPanId = 0x7777;
/* The current logical channel (frequency band) */
static uint8_t mLogicalChannel = 11;
...
/* Table used to retain association information for up to five devices. */
static tCoordinatorNodeInfo_t mCoordinatorNodeTable[mCoordinatorMaxNodes_c];
/* Next short address to assign to a new device. */
static uint16_t mCoordinatorNextShortAddress = 0x0001;
...
void App_init( void )
{
    ...
    mcPendingPackets = 0;
    /* Reset the registered-device table. */
    FLib_MemSet(mCoordinatorNodeTable, 0, sizeof(mCoordinatorNodeTable));
    mCoordinatorNextShortAddress = 0x0001;
...
/******************************************************************************
*************************************************************************************
* Functions used to register and retain information about associated devices.
*************************************************************************************
************************************************************************************/
static int8_t Coordinator_FindByExtended(uint64_t extendedAddress)
{
    uint8_t index;
    for(index = 0; index < mCoordinatorMaxNodes_c; index++)
    {
        if(mCoordinatorNodeTable[index].inUse &&
           (mCoordinatorNodeTable[index].extendedAddress == extendedAddress))
        {
            return (int8_t)index;
        }
    }
...
static uint8_t Coordinator_RegisterNode(uint64_t extendedAddress, uint8_t capabilityInfo, uint16_t *assignedShortAddress)
{
    ...
    uint8_t newIndex;
    if(assignedShortAddress == NULL)
    {
        return FALSE;
    }
    existingIndex = Coordinator_FindByExtended(extendedAddress);
    if(existingIndex >= 0)
    {
        /* Existing extended addresses keep their original short address. */
        *assignedShortAddress = mCoordinatorNodeTable[(uint8_t)existingIndex].shortAddress;
        mCoordinatorNodeTable[(uint8_t)existingIndex].rxOnWhenIdle =
            (capabilityInfo & 0x08) ? TRUE : FALSE;
        mCoordinatorNodeTable[(uint8_t)existingIndex].deviceType =
            (capabilityInfo & 0x02) ? TRUE : FALSE;
        return TRUE;
    }
    for(newIndex = 0; newIndex < mCoordinatorMaxNodes_c; newIndex++)
    {
        if(!mCoordinatorNodeTable[newIndex].inUse)
        {
            break;
        }
    }
    if(newIndex >= mCoordinatorMaxNodes_c)
    {
        return FALSE;
    }
    if(capabilityInfo & gCapInfoAllocAddr_c)
    {
        mCoordinatorNodeTable[newIndex].shortAddress = mCoordinatorNextShortAddress++;
    }
    else
    {
        mCoordinatorNodeTable[newIndex].shortAddress = 0xFFFE;
    }
    mCoordinatorNodeTable[newIndex].extendedAddress = extendedAddress;
    mCoordinatorNodeTable[newIndex].rxOnWhenIdle =
        (capabilityInfo & 0x08) ? TRUE : FALSE;
    mCoordinatorNodeTable[newIndex].deviceType =
        (capabilityInfo & 0x02) ? TRUE : FALSE;
    mCoordinatorNodeTable[newIndex].inUse = TRUE;
    *assignedShortAddress = mCoordinatorNodeTable[newIndex].shortAddress;
    return TRUE;
}
static void Coordinator_PrintNode(uint8_t nodeIndex)
{
    Serial_Print(interfaceId, "\r\n[Association] Node ", gAllowToBlock_d);
    Serial_PrintHex(interfaceId, &nodeIndex, 1, gPrtHexNoFormat_c);
    Serial_Print(interfaceId, " | Short: 0x", gAllowToBlock_d);
    Serial_PrintHex(interfaceId,
                    (uint8_t*)&mCoordinatorNodeTable[nodeIndex].shortAddress,
                    2, gPrtHexNoFormat_c);
    Serial_Print(interfaceId, " | Extended: 0x", gAllowToBlock_d);
    Serial_PrintHex(interfaceId,
                    (uint8_t*)&mCoordinatorNodeTable[nodeIndex].extendedAddress,
                    8, gPrtHexNoFormat_c);
    Serial_Print(interfaceId, " | RxOnWhenIdle: ", gAllowToBlock_d);
    Serial_PrintHex(interfaceId, &mCoordinatorNodeTable[nodeIndex].rxOnWhenIdle, 1, gPrtHexNoFormat_c);
    Serial_Print(interfaceId, " | DeviceType: ", gAllowToBlock_d);
    if(mCoordinatorNodeTable[nodeIndex].deviceType)
    {
...
static uint8_t App_SendAssociateResponse(nwkMessage_t *pMsgIn, uint8_t appInstance)
{
  ...
  uint64_t incomingExtendedAddress;
  uint8_t incomingCapabilityInfo;
  ...
  if(pMsg != NULL)
  {
    ...
    /* Assign a short address to the device. In this example we simply
       ...
       be assigned to it. */
    FLib_MemCpy(&incomingExtendedAddress, &pMsgIn->msgData.associateInd.deviceAddress, 8);
    incomingCapabilityInfo = pMsgIn->msgData.associateInd.capabilityInfo;
    if(Coordinator_RegisterNode(incomingExtendedAddress, incomingCapabilityInfo, &assignedShortAddress))
    {
        pAssocRes->assocShortAddress = assignedShortAddress;
    ...
    /* The status was selected above according to the available table entry. */
    /* Do not use security */
    pAssocRes->securityLevel = gMacSecurityNone_c;
    /* Save device info. */
    if(pAssocRes->status == gSuccess_c)
    {
    ...
    if(pAssocRes->status == gSuccess_c)
    {
        int8_t nodeIndex = Coordinator_FindByExtended(incomingExtendedAddress);
        if(nodeIndex >= 0)
        {
            Coordinator_PrintNode((uint8_t)nodeIndex);
        }
    }
...
static void App_HandleMcpsInput(mcpsToNwkMessage_t *pMsgIn, uint8_t appInstance)
{
  ...
  case gMcpsDataCnf_c:
    ...
    break;
  case gMcpsDataInd_c:
    /* Imprimir detalles de la telemetria para el reporte */
    Serial_Print(interfaceId, "\r\n[RX Data] From: 0x", gAllowToBlock_d);
    ...
    Serial_Print(interfaceId, " | Payload: ", gAllowToBlock_d);
    ...
    Serial_Print(interfaceId, "\r\n", gAllowToBlock_d);
    /* Cambiar el LED RGB del Coordinador segun el contador recibido */
    if(pMsgIn->msgData.dataInd.msduLength >= 10)
    {
        uint8_t counterVal = pMsgIn->msgData.dataInd.pMsdu[9] - '0';
        TurnOffLeds();
        /* board.h maps the RGB LED as LED2=Red, LED3=Green, LED4=Blue */
        switch(counterVal % 4)
        {
            case 0: LED_TurnOnLed(LED3); break; /* Verde */
            case 1: LED_TurnOnLed(LED2); break; /* Rojo */
            case 2: LED_TurnOnLed(LED4); break; /* Azul */
            case 3:                             /* Magenta */
                LED_TurnOnLed(LED2);
                LED_TurnOnLed(LED4);
                break;
