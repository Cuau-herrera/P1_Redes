...
/* Fase 1: contador periodico + LED RGB + envio "Counter: x" */
static void    App_CounterTimerCallback(void *param);
static void    App_SendCounterPacket(uint8_t counter);
static void    App_UpdateCounterLed(uint8_t counter);
...
/* Fase 1: timer de 4s, valor actual del contador [0-3], y override por switch */
static tmrTimerID_t mCounterTimerId = gTmrInvalidTimerID_c;
static uint8_t mCounterValue = 0;
static uint8_t mSwitchOverrideValue;
#define mCounterPollInterval_c 4000
    ...
    while(1)
    {
        ...
        case stateInit:
            ...
            if ( mCoordInfo.coordAddress && ( mAddrMode != gAddrModeNoAddress_c ) ) 
            {
                ...
                /* Fase 1: iniciar el timer de 4s del contador tambien en este camino (restore NVM) */
                mCounterTimerId = TMR_AllocateTimer();
                mCounterValue = 0;
                App_UpdateCounterLed(mCounterValue);
                TMR_StartIntervalTimer(mCounterTimerId, mCounterPollInterval_c, App_CounterTimerCallback, NULL);
                gState = stateListen;
            }
            else
            {
                ...
                {   
                    ...
                    {          
                        ...
                        { 
                            ...
                            /* Startup the timer */
                            TMR_StartLowPowerTimer(mTimer_c, gTmrSingleShotTimer_c ,mPollInterval, AppPollWaitTimeout, NULL );
                            /* Fase 1: iniciar el timer de 4s del contador */
                            mCounterTimerId = TMR_AllocateTimer();
                            mCounterValue = 0;
                            App_UpdateCounterLed(mCounterValue);
                            TMR_StartIntervalTimer(mCounterTimerId, mCounterPollInterval_c, App_CounterTimerCallback, NULL);
        ...
        case stateListen:
            ...
            /* Fase 1: contador periodico de 4s */
            if (ev & gAppEvtCounterTimerExpired_c)
            {
                mCounterValue = (mCounterValue + 1) % 4;
                App_UpdateCounterLed(mCounterValue);
                App_SendCounterPacket(mCounterValue);
            }
            /* Fase 1: override inmediato por switch, reinicia el timer de 4s */
            if (ev & gAppEvtSwitchOverride_c)
            {
                mCounterValue = mSwitchOverrideValue;
                App_UpdateCounterLed(mCounterValue);
                App_SendCounterPacket(mCounterValue);
                TMR_StartIntervalTimer(mCounterTimerId, mCounterPollInterval_c, App_CounterTimerCallback, NULL);
            }
...
/******************************************************************************
* Fase 1: reflects mCounterValue on the RGB LED (0=Green,1=Red,2=Blue,3=Magenta)
******************************************************************************/
static void App_UpdateCounterLed(uint8_t counter)
{
    TurnOffLeds();
    /* board.h mapea el LED RGB como LED2=Rojo, LED3=Verde, LED4=Azul */
    switch(counter % 4)
    {
...
/******************************************************************************
* Fase 1: fires every 4 seconds to advance the counter
******************************************************************************/
static void App_CounterTimerCallback(void *param)
{
    (void)param;
    OSA_EventSet(mAppEvent, gAppEvtCounterTimerExpired_c);
}

/******************************************************************************
* Fase 1: builds and sends the "Counter: x" MCPS-Data request to the coordinator
******************************************************************************/
static void App_SendCounterPacket(uint8_t counter)
{
    nwkToMcpsMessage_t *pCounterPacket;
    uint8_t payload[10] = "Counter: 0";
    payload[9] = (uint8_t)('0' + (counter % 4));

    if(mcPendingPackets < mDefaultValueOfMaxPendingDataPackets_c)
    {
        pCounterPacket = MSG_Alloc(sizeof(nwkToMcpsMessage_t) + sizeof(payload));
        if(pCounterPacket != NULL)
        {
            pCounterPacket->msgType = gMcpsDataReq_c;
            pCounterPacket->msgData.dataReq.pMsdu = (uint8_t*)(&pCounterPacket->msgData.dataReq.pMsdu) +
                sizeof(pCounterPacket->msgData.dataReq.pMsdu);
            FLib_MemCpy(pCounterPacket->msgData.dataReq.pMsdu, payload, sizeof(payload));
            FLib_MemCpy(&pCounterPacket->msgData.dataReq.dstAddr, &mCoordInfo.coordAddress, 8);
            FLib_MemCpy(&pCounterPacket->msgData.dataReq.srcAddr, &maMyAddress, 8);
            FLib_MemCpy(&pCounterPacket->msgData.dataReq.dstPanId, &mCoordInfo.coordPanId, 2);
            FLib_MemCpy(&pCounterPacket->msgData.dataReq.srcPanId, &mCoordInfo.coordPanId, 2);
            pCounterPacket->msgData.dataReq.dstAddrMode = mCoordInfo.coordAddrMode;
            pCounterPacket->msgData.dataReq.srcAddrMode = mAddrMode;
            pCounterPacket->msgData.dataReq.msduLength = sizeof(payload);
            pCounterPacket->msgData.dataReq.txOptions = gMacTxOptionsAck_c;
            pCounterPacket->msgData.dataReq.msduHandle = mMsduHandle++;
            pCounterPacket->msgData.dataReq.securityLevel = gMacSecurityNone_c;
            (void)NWK_MCPS_SapHandler(pCounterPacket, macInstance);
            mcPendingPackets++;
        }
    }
    Serial_Print(interfaceId, "\r\nSent: ", gAllowToBlock_d);
    Serial_SyncWrite(interfaceId, payload, sizeof(payload));
    Serial_Print(interfaceId, "\r\n", gAllowToBlock_d);
}
  ...
  )
{
    ...
    switch ( events ) 
    { 
    /* Unico switch fisico disponible (gKBD_KeysCount_c=2): SW1_c/SW2_c.
       Verificar en la tarjeta cual botón corresponde a "SW3"/"SW4" del enunciado. */
    case gKBD_EventSW1_c:
        if(gState == stateListen)
        {
            mSwitchOverrideValue = 0; /* SW3 -> Counter = 0 */
            OSA_EventSet(mAppEvent, gAppEvtSwitchOverride_c);
            break;
        }
        if(gState == stateInit)
        {
    ...
    case gKBD_EventSW2_c:
        if(gState == stateListen)
        {
            mSwitchOverrideValue = 2; /* SW4 -> Counter = 2 */
            OSA_EventSet(mAppEvent, gAppEvtSwitchOverride_c);
            break;
        }
        if(gState == stateInit)
